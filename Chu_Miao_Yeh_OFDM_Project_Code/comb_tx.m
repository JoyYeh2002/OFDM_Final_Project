...
%% Comb Type Digital Transmitter
function [txsymbols, txsignal_complex, txsignal, conf] = comb_tx(txbits,conf,k)
% Digital Transmitter
%
...
txsymbols = GreyMap_QPSK(bi2de(txbits,'left-msb')+1); % [NEW] Also return this
...
% S/P: paralize training and data symbols
...
for i = 0:conf.nsyms-1
    ...
    %     if i==0
    %         conf.before = IFFTsymbol; % for debugging
    %     end
...
% figure()
% TX = fft(time_OFDMsymbols);
% t = (0:length(time_OFDMsymbols)-1) / length(time_OFDMsymbols)* conf.f_s;
% plot(t,abs(TX))
...
% [New]: Save the txsignal_complex (1072000 x 1 complex double) AND the
% txsignal with only the real part.
